...
const clamp=(x,a,b)=>x<a?a:x>b?b:x;let W=0,H=0,dpr=1;const V={lon:135,lat:22,s:10};
...
cv.onpointerdown=e=>{cv.setPointerCapture(e.pointerId);ptr.set(e.pointerId,[e.clientX,e.clientY]);pd=pinch()};
...
cv.onpointerup=cv.onpointercancel=e=>{ptr.delete(e.pointerId);pd=null};
...
const L={wind:{lo:0,hi:70,u:'m/s',st:[[0,[25,40,100]],[.15,[40,120,200]],[.3,[80,200,120]],[.45,[240,220,60]],[.65,[240,120,40]],[.85,[220,40,60]],[1,[200,60,200]]]},
...
let lvU=F.u,lvV=F.v,base,layer='wind',speed=6,acc=0,last=performance.now(),fr=0,dirty=true,isoT=0,iso=[];
...
function updOverlay(){const l=L[layer];if(!l)return;const d=oi.data;
  for(let k=0;k<GW*GH;k++){const val=layer==='wind'?Math.hypot(lvU[k],lvV[k]):layer==='sst'?F.sst0[k]+F.sa[k]:layer==='pressure'?F.p[k]:F.q[k],
    i=clamp(((val-l.lo)/(l.hi-l.lo)*255)|0,0,255)*3;d[k*4]=l.lut[i];d[k*4+1]=l.lut[i+1];d[k*4+2]=l.lut[i+2];d[k*4+3]=layer==='sst'&&F.el[k]>0?0:150}
  og.putImageData(oi,0,0)}
// isobars via marching squares (4 hPa), segments in grid coords
function calcIso(){const s=[],p=F.p,st=Math.max(1,Math.round(.25/D)),G=GW*st;   // isobars on a 0.25 deg subsample (smooth field, keeps it cheap at 0.125 deg)
  for(let j=0;j<GH-st;j+=st)for(let i=0;i<GW-st;i+=st){const k=j*GW+i,a=p[k],b=p[k+st],c=p[k+G+st],d=p[k+G],mn=Math.min(a,b,c,d),mx=Math.max(a,b,c,d);
...
function nodes(){const lab=V.s>=14;g.font='bold 10px sans-serif';
  for(const s of S.hist.concat(S.storms)){if(!s.ev)continue;g.globalAlpha=S.hist.includes(s)?.55:1;
...
function tracks(){if(S.hm&&TS.hind)for(const o of TS.hind.storms){if(!o.done)continue;g.save();g.setLineDash([5,4]);g.strokeStyle='rgba(255,255,255,.75)';g.lineWidth=1.6;g.beginPath();o.trk.forEach((p,i)=>i?g.lineTo(X(p[1]),Y(p[2])):g.moveTo(X(p[1]),Y(p[2])));g.stroke();g.restore()}
  for(const s of S.hist.concat(S.storms)){g.globalAlpha=S.hist.includes(s)?.4:1;g.lineWidth=2.2;
  ...
  for(const s of S.storms){const x=X(s.lon),y=Y(s.lat),c=cat(s.v)[2],r=8+s.v/8,a=fr*.06;g.fillStyle=c+'44';g.strokeStyle=c;g.lineWidth=2.5;g.beginPath();g.arc(x,y,r,0,7);g.fill();
    ...
    for(const[t,col]of[[L1,'#fff'],[L2,c],[L3,'#cfe8ff'],[L4,'#ffb347']]){if(!t)continue;g.fillStyle=col;g.strokeText(t,tx,ty);g.fillText(t,tx,ty);ty+=13}}}
function ui(){const d=new Date(S.t0+S.t*36e5);$('clock').textContent=`${d.toISOString().slice(0,16).replace('T',' ')}Z  (T+${S.t}h) · ${S.E.enso>.3?'El Niño':S.E.enso<-.3?'La Niña':'ENSO-neutral'} · ${S.mode()}`;
...
function loop(now){const dt=Math.min(.1,(now-last)/1000);last=now;
  if(S.running){acc+=dt*speed;const t0=performance.now();let n=0;while(acc>=1&&n<48&&performance.now()-t0<14){S.step();acc--;n++;dirty=true}if(acc>3)acc=3}
  if(dirty){updOverlay();if(now-isoT>250){calcIso();isoT=now;dirty=false}}
  g.drawImage(base,0,0,C.bw,C.bh,X(C.lon0),Y(C.lat1),90*V.s*KX,55*V.s);
  if(L[layer])g.drawImage(ov,0,0,GW,GH,X(C.lon0),Y(C.lat1),90*V.s*KX,55*V.s);
  if($('iso').checked)drawIso();const wm=$('wmode').value;if(wm==='particles')particles();else if(wm==='arrows')arrows();
  tracks();if(++fr%8==0)ui();requestAnimationFrame(loop)}
$('start').onclick=()=>{S.running=!S.running;$('start').textContent=S.running?'⏸ Pause':'▶ Start Sim'};
$('reset').onclick=()=>{S.reset();$('start').textContent='▶ Start Sim';dirty=true;ui()};
...
$('ptoggle').onclick=()=>$('panel').classList.toggle('hide');addEventListener('resize',resize);
S.t0=Date.parse($('date').value+'T00:00:00Z');resize();V.s=minS();fixV();
TS.loadTerrain(p=>$('loading').textContent=`Loading terrain… ${(p*100)|0}%`).then(async el=>{await TS.loadClim('js/clim.bin');await TS.loadStats('js/stats.json');await TS.loadHind('js/hind.bin','js/hind.json');$('wx').disabled=!TS.hind;document.querySelector('.note').insertAdjacentHTML('beforeend','<br>Mean flow: '+(TS.clim?'reanalysis climatology':'analytic (run tools/make_clim.py)')+'; anomalies: barotropic model.'+(TS.stats?' Genesis from IBTrACS.':'')+(TS.hind?' Hindcast window: '+TS.hind.start+' + '+TS.hind.days+' d (dates outside it fall back to climatology + stochastic genesis).':' No hindcast loaded — Weather stays on Synthetic (see console for why; regenerate with tools/make_hindcast.py).'));
